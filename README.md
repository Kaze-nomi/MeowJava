...
▎Swagger + @Operation, OpenAPI:  
   Swagger — это инструмент для документирования и тестирования API. OpenAPI — это стандарт описания API, который поддерживает Swagger. Аннотация @Operation позволяет описывать методы API (например, их назначение). Пример: Swagger генерирует интерфейс, где можно протестировать запросы к вашему API.
